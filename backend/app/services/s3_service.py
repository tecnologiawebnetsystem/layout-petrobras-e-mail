...
# Config — lê via Settings (que já absorveu Parameter Store / .env)
from app.core.config import settings as _settings
AWS_REGION: str = _settings.aws_region or os.getenv("AWS_REGION", "sa-east-1")
S3_BUCKET: str  = _settings.aws_s3_bucket or os.getenv("AWS_S3_BUCKET", "s3-a12022-dsv-a12022-s3-119392112451-sa-east-1")
 
def get_s3_client() -> BaseClient:
    """Retorna o client S3 usando a cadeia de credenciais padrão do boto3.

    Ordem de resolução (automática pelo boto3, sem código explícito):
      1. Variáveis de ambiente (AWS_ACCESS_KEY_ID / SECRET / SESSION_TOKEN) → dev local / CI
      2. IAM Role da ECS Task / EC2 Instance Profile → produção AWS
