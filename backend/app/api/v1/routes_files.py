...
async def upload_files(
    ...
    try:
        ...
        for file in files:
            ...
            key_s3 = build_upload_key(str(area.id), file_uuid, safe_name)
            ...
            if settings.storage_provider == "aws":
                ...
                # dev local: salva em disco com path posix para consistência
                import posixpath as _pp
                area_path = STORAGE_ROOT / area.prefix_s3
                area_path.mkdir(parents=True, exist_ok=True)
                file_path = area_path / f"{share.id}_{safe_name}"
                file_path.write_bytes(content)
                key_s3 = _pp.join(area.prefix_s3, f"{share.id}_{safe_name}")
            ...
            share_file = ShareFile(share_id=share.id, file_id=rfile.id)
        ...
        for k in uploaded_keys:
            try:
                ...
                pass
        session.rollback()
        raise HTTPException(status_code=502, detail=f"Falha ao enviar arquivo para o S3: {exc}")
