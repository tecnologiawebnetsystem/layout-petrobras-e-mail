...
# ─────────────────────────────────────────────────────────────────────────────
# Fixtures Entra ID (para testes de autenticacao backend-driven)
# ─────────────────────────────────────────────────────────────────────────────
...
@pytest.fixture()
def jwt_interno(usuario_interno) -> str:
    """JWT valido emitido pelo backend para usuario interno."""
    return create_session_jwt(
        user_id=usuario_interno.id,
        email=usuario_interno.email,
        user_type=usuario_interno.type,
        is_supervisor=False,
        expires_minutes=60,
    )
...
@pytest.fixture()
def jwt_supervisor(usuario_supervisor) -> str:
    """JWT valido emitido pelo backend para usuario supervisor."""
    return create_session_jwt(
        user_id=usuario_supervisor.id,
        email=usuario_supervisor.email,
        user_type=usuario_supervisor.type,
        is_supervisor=True,
        expires_minutes=60,
    )
...
@pytest.fixture()
def jwt_expirado(usuario_interno) -> str:
    """JWT ja expirado (0 minutos)."""
    return create_session_jwt(
        user_id=usuario_interno.id,
        email=usuario_interno.email,
        user_type=usuario_interno.type,
        is_supervisor=False,
        expires_minutes=-1,  # ja expirou
    )


@pytest.fixture()
def refresh_token_valido(session, usuario_interno) -> str:
    """Refresh token valido armazenado no banco."""
...
@pytest.fixture()
def refresh_token_expirado(session, usuario_interno) -> str:
...
@pytest.fixture()
def refresh_token_revogado(session, usuario_interno) -> str:
...
@pytest.fixture()
def usuario_inativo(session) -> User:
    """Usuario com status=False (desativado)."""
    user = User(
        name="Inativo Dev",
        email="inativo.dev@petrobras.com.br",
        type=TypeUser.INTERNAL,
        status=False,
    )
    session.add(user)
...
@pytest.fixture()
def jwt_usuario_inativo(usuario_inativo) -> str:
    """JWT para usuario desativado."""
    return create_session_jwt(
        user_id=usuario_inativo.id,
        email=usuario_inativo.email,
        user_type=usuario_inativo.type,
        is_supervisor=False,
        expires_minutes=60,
    )
