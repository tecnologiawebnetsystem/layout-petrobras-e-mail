...
export function LoginForm() {
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.detail ?? data.error?.message ?? "Erro ao enviar codigo")
      ...
      setNotification({
        ...
        message: error.message || "Nao foi possivel enviar o codigo. Tente novamente.",
      })
    } finally {
      setIsLoading(false)
    }
    ...
    if (index === 5 && value) {
      ...
      try {
        ...
        setNotification({
          ...
          message: error.message || "O codigo informado esta incorreto. Tente novamente.",
        })
        setVerificationCode(["", "", "", "", "", ""])
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.detail ?? data.error?.message ?? "Erro ao reenviar codigo")
      if (data.code) setGeneratedCode(data.code)
      setNotification({
        show: true,
        type: "info",
      ...
      setNotification({
        ...
        message: resendError instanceof Error ? resendError.message : "Nao foi possivel reenviar o codigo. Tente novamente.",
      })
    } finally {
  ...
  const handleEntraIdLogin = async () => {
    ...
    try {
      ...
      // Redireciona para o backend, que conduz todo o fluxo OAuth2
      // Authorization Code com o Microsoft Entra ID.
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Usuarios internos: nao ha login local. Exibe mensagem orientando uso do botao Microsoft.
    setNotification({
      show: true,
      type: "info",
      title: "Use o Login com Microsoft",
      message: "Colaboradores Petrobras devem usar o botao 'Login com Microsoft' abaixo para acessar o sistema.",
    })
  }
    ...
    <main className="min-h-screen flex" role="main" aria-label="Pagina de login">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-8 lg:p-12 bg-background">
        <div className="w-full max-w-md space-y-8">
                  ...
                  <div className="bg-amber-50 dark:bg-amber-950/20 border-2 border-amber-200 dark:border-amber-900/50 rounded-lg p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-amber-900 dark:text-amber-100">Codigo (apenas em dev):</p>
          ...
          {/* Botao Login com Microsoft (Entra ID via backend) */}
          <div className="space-y-4">
            <div className="relative">
            ...
            >
              ...
              {isLoading ? "Redirecionando..." : "Login com Microsoft"}
            </Button>
          </div>
