...
export function LoginForm() {
  ...
  /**
   * Credenciais locais de suporte (temporario - sem consulta ao backend).
   * Substituir por autenticacao real em etapa futura.
   */
  const SUPPORT_CREDENTIALS: Record<string, { password: string; name: string; role: "support" | "supervisor" }> = {
    "suporte@petrobras.com.br": { password: "suporte@123", name: "Suporte Atendimento", role: "support" },
  }

  const handleSupportLogin = async () => {
    if (!email || !password) {
      setNotification({ show: true, type: "warning", title: "Campos obrigatorios", message: "Preencha o e-mail e a senha." })
      return
    }
    setIsLoading(true)
    // Simula delay de autenticacao
    await new Promise(resolve => setTimeout(resolve, 500))
    const credential = SUPPORT_CREDENTIALS[email.toLowerCase()]
    if (!credential || credential.password !== password) {
      setIsLoading(false)
      setNotification({ show: true, type: "error", title: "Acesso negado", message: "Credenciais invalidas ou usuario sem perfil de Suporte." })
      return
    }
    setAuth(
      { id: email.toLowerCase(), email: email.toLowerCase(), name: credential.name, userType: credential.role },
      `demo_access_${Date.now()}`,
    ...
    <main className="min-h-screen flex" role="main" aria-label="Pagina de login">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-8 lg:p-12 bg-background">
        <div className="w-full max-w-md space-y-8">
          ...
          {/* Botao Acesso Suporte - usa os campos email+senha do formulario principal */}
          {/* <div className="pt-2">
            <Button
              type="button"
              variant="ghost"
            ...
            >
              ...
              {isLoading ? "Autenticando..." : "Acesso Suporte"}
            </Button>
          </div> */}
