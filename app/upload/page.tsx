  ...
  const handleFilesSelected = async (newFiles: File[]) => {
    ...
    if (blockedFiles.length > 0) {
      setNotification({
        ...
        message: `Os seguintes arquivos não podem ser enviados: ${blockedFiles.join(", ")}. Extensões bloqueadas: .exe, .dll, .bat, .cmd, .com, .msi, .scr, .vbs, .ps1, .sh`,
      })
      return
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!solicitacaoId) {
      setNotification({
        show: true,
        type: "warning",
        title: "Solicitação não selecionada",
        message: "Por favor, selecione o número da solicitação antes de continuar.",
      })
      return
    }
                  ...
                  <div className="flex items-start gap-3 stat-card-orange border border-amber-200 rounded-xl p-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-amber-700 leading-relaxed">
                        Não existe nenhuma solicitação aberta vinculada ao seu e-mail. Entre em contato com o
                        Suporte para que uma solicitação seja cadastrada antes de prosseguir.
                      </p>
                    </div>
                  </div>
                ) : (
                ...
                <div className="stat-card-blue border border-[#0066CC]/20 rounded-xl p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-background/70 border border-border/50 rounded-lg px-4 py-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-background/70 border border-border/50 rounded-lg px-4 py-3">
                        <p className="font-medium text-foreground text-sm">{recipient}</p>
                        <p className="text-xs text-muted-foreground">Não pode ser alterado</p>
                      </div>
                    </div>
                  </div>

                  {/* Input oculto para manter o valor de recipient no submit */}
                  <Input type="hidden" value={recipient} readOnly />
                </div>
              )}

              {/* Só exibe o restante do formulário após selecionar uma solicitação */}
              {solicitacaoSelecionada && (
                <>
                  {/* Arquivos */}
                  <div className="space-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Os arquivos ficarão disponíveis por {expirationHours} horas após a aprovação. Máximo: 168 horas (7 dias).
                    </p>
                  </div>
