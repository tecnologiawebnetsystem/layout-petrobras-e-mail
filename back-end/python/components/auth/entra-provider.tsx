...
export function EntraProvider({ children }: EntraProviderProps) {
  ...
  const handleLoginSuccess = async (response: AuthenticationResult) => {
    ...
    console.log(" Account no response:", !!response?.account)
    ...
    console.log(" ===== DADOS RECEBIDOS DO ENTRA ID TOKEN =====")
    ...
    try {
      console.log(" Buscando perfil completo do usuário via Graph API...")
      const [profile, manager, photo] = await Promise.all([getUserProfile(), getUserManager(), getUserPhoto()])

      console.log(" ===== DADOS RECEBIDOS DO GRAPH API =====")
      console.log(
        " Perfil completo:",
        profile
          ? {
      ...
      console.log(" Foto do perfil:", photo ? "RECEBIDA (URL blob)" : "NÃO RECEBIDA")
      console.log(" ==========================================")
    ...
    console.log(" UserType determinado:", userType, "- Email:", email, "- JobTitle:", jobTitle)
    ...
    console.log(" Login processado com sucesso, usuário autenticado")

    // Redirecionar baseado no tipo de usuário
    const redirectPath = userType === "supervisor" ? "/supervisor" : userType === "internal" ? "/upload" : "/download"
    console.log(" Redirecionando para", redirectPath, "- userType:", userType)
  ...
  useEffect(() => {
    ...
    const handleCustomLoginEvent = async (event: Event) => {
      ...
      console.log(" Evento customizado de login recebido no provider")
      if (customEvent.detail?.response) {
        console.log(" Processando login do Entra ID, substituindo usuário atual se houver")
        await handleLoginSuccess(customEvent.detail.response)
      }
    }

    window.addEventListener("msal-login-success", handleCustomLoginEvent)
    console.log(" Listener de evento msal-login-success registrado")
    ...
    const initializeMsal = async () => {
      try {
        ...
        console.log(" Contas encontradas no MSAL após inicialização:", accounts.length)

        const currentUser = useAuthStore.getState().user
        console.log(
          " Usuário atual no store:",
          currentUser
            ? {
        ...
        if (accounts.length > 0) {
          console.log(" Conta MSAL encontrada, enriquecendo perfil")
          console.log(" Email da conta:", accounts[0].username)
          console.log(" Nome da conta:", accounts[0].name)
          console.log(" Tentando adquirir token silencioso com scopes:", loginRequest.scopes)
          ...
          try {
            ...
            console.log(" Token silencioso adquirido com sucesso")
            await handleLoginSuccess(response)
          } catch (error) {
            console.error(" Erro ao adquirir token silencioso:", error)
            ...
            console.log(" Tentando login popup interativo")
            try {
              ...
              console.error(" Erro no login popup:", popupError)
            }
          }
        }
        ...
        try {
          ...
          console.log(" handleRedirectPromise concluído:", !!response)
          if (response !== null) {
            console.log(" Response de redirect encontrado, processando")
            await handleLoginSuccess(response)
          }
        } catch (error: any) {
