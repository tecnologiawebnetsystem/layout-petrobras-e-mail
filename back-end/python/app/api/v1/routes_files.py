...
def create_metadata(payload: FileCreate, session: Session = Depends(get_session), request: Request = None):
    ...
    rfile = RestrictedFile(
        ...
        expira_at=payload.expira_at, # Campo extra caso no futuro a gente evolua na aplicação para validação tb ser por arquivo (caso muito provavel que não exista)
        status=True
    )
    session.add(rfile)
    session.commit()
